...
import { ConnectApp, ConnectCard, SignIn, Unreachable } from './flows.tsx';

type View = 'home' | 'chief' | 'crew' | 'add' | 'helper' | 'things' | 'routines' | 'settings' | 'apps' | 'ask';
...
function Hello({ state, me, refresh }: Ctx) {
  ...
  const need = () => { if (name.trim()) return true; toast('First, what shall I call you?'); input.current?.focus(); return false; };
  const finish = () => attempt(async () => { await api.onboard(name.trim()); refresh(); });
  return (
    <div className="hello">
      <span className="halo"><ChiefArt mood="idle" d={8.5} /></span>
      <h1>{A.greeting()}. I'm Chief.</h1>
      <p className="lead">I run the crew in this house. How may I be of service? First, what shall I call you?</p>
      <input ref={input} className="input name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name" aria-label="What shall I call you?" autoFocus />
      <div className="chips center">{['Sir', "Ma'am"].map((q) => <button key={q} className={`chip ${name === q ? 'on' : ''}`} onClick={() => setName(q)}>{q}</button>)}</div>
      <div className="card promises">
        <div>🔒 I always ask before anything leaves the house or costs money.</div>
...
// ---------- a chat ----------
function Chat({ id, state, tick, refresh }: Ctx & { id: string }) {
  ...
  return (
    <div className="chat">
      <div className="lines">
        ...
        {cards.map((c) => c.kind === 'connect' ? <ConnectCard key={c.id} c={c} helper={h?.name} onDone={refresh} /> : <AskCard key={c.id} c={c} who={h} onDone={refresh} />)}
...
function Apps({ state, refresh }: Ctx) {
  ...
  const [connecting, setConnecting] = useState<A.App | null>(null);
  return (
    <div className="page">
      ...
      {connecting && <ConnectApp app={connecting} onClose={() => setConnecting(null)} onDone={() => { setConnecting(null); refresh(); }} />}
    </div>
  );
}
