...
- Spend-bearing tool commands go under `ask` in `tools/<id>/tool.json` (Claude's `permissions.ask`, which beats allow and fires the `PermissionRequest` hook), never under `allow`. See `tools/people-search`. crewd's own standing answers ("For this task", "Always for <bot>", `permissionRule`/`mustAsk` in `src/bots.ts`) never cover an `ask` rule or a browser asks-first gate, and a compound shell command only ever earns an exact-command grant. Chained commands (`cd x && …`, loops, pipes) never match a `Bash(tool *)` allow rule either, so worker templates ask bots for one plain command at a time; a bot that chains floods Needs you.
...
- Restart is a non-event: in-memory state in `Crew` (holds, live runs, handoffs) must be rebuildable from SQLite plus Herdr. `Crew.recover()` re-attaches tasks whose pane is still alive and resumes the rest from a continuation brief; `bin/crew` retries while crewd is down (just under each hook's timeout), so anything a hook or `crew` call does must be safe to receive twice. `test/restart.test.ts` kills a real crewd mid-run.
