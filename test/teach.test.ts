...
test('a real show: what was clicked and which box was typed in, never the words, nothing from a password box', { skip: !browser && 'no Chromium here' }, async () => {
  ...
  // Its port opens before its first page is listed: wait for the page itself.
  const pages = () => fetch(`http://127.0.0.1:${port}/json/list`).then((r) => r.json(), () => []).then((l: any[]) => l.filter((x) => x.type === 'page' && x.webSocketDebuggerUrl));
  await until('chromium', async () => (await pages()).length > 0);
