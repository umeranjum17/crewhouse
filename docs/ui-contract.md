...
| `routines[]` `{name, words, next_at, state, kind, quiet, watch, history}` | Routines | `quiet`: a check-in that only speaks up when something needs the person ("Only tell me if something's up"); a history run with `clear` found nothing. `watch`: the page a watch keeps an eye on (the app shows only its host); a watch's history runs carry `watch`: `started`, `same` (no change, no AI used), `changed` (with the helper's `task`) or `unreachable`. A history run also carries `thing` (its task delivered a file) or `msg` (its last line in the helper's chat), which the app turns into the row's **See result**. Recurring work starts as a request: Chief's `crew_routine` opens a `propose` ask whose detail carries `routine` (the raw request, `schedule` included, so the card's **Change time** can edit it) and a `preview` with the confirmation lines (cadence, what, quiet behaviour, first run); `POST /api/asks/:id/answer {answer:'allow', schedule}` starts it with a changed time. `POST /api/routines` still takes `watch`, and then `task` may be empty. `thinks` (an account name) is never read |
| `zone` | Naming the time zone | The home computer's IANA zone. The app names it ("Times follow the home computer's clock (Asia/Karachi).") only when the device's zone differs |
...
A checkout page (a `spend` from the browser) carries `preview` `{head, body}`: the order's lines as the page writes them and its total, read by crewd from the browser tool's own page snapshot, never the model's words, with the shop's host only. Its total is the spend's cost toward the monthly cap when it is in dollars; a price in another currency says so on the card and never counts, and when the total can't be read, the words say so and it doesn't count. One yes covers that page's later clicks until the page or the total changes. A checkout ask also carries `order` `{shown, known, dollars}` for the review sheet: `shown` is the total as the page writes it, `known` whether crewd could read one, `dollars` whether it counts toward the cap — never the page's address. Cards for a checkout are review-first: the inbox opens the review and offers "Don't place order"; the sheet's yes names the order ("Place order · $43.10"); with no readable total there is no yes — "Don't place order" and "I'll buy it myself".
...
A delivered workbook (`*.xlsx`, made by `crew_workbook`) rides in the chat as `Delivered files/<name>.xlsx: N sheets: …`, shown as a workbook card: its name, how many sheets, a peek at the first sheet's headings, **Open**. `GET /api/workbook?bot&path` reads it back as words only — `{sheets: [{name, total, rows[]}]}`, heading row first, at most 40 rows × 14 columns per sheet, formulas as their computed value or a quiet `—` — parsed crewd-side with crewd's own exceljs; the app renders it read-only (sheet tabs, a table, **Download**), never the file. Only a member the file was delivered to may read it (403 otherwise). `#/f/<helper>/<file>` opens the panel beside the chat that delivered it: side panel on a desk, whole screen on a phone.

The same shape of story for documents: a delivered `*.docx` (made by `crew_document`) is a card too, and `GET /api/document?bot&path` returns `{parts: [{kind: 'heading'|'p'|'li'|'table', text, bold?, head?, rows?}]}` — plain words crewd parsed out of the file (docx + its own jszip/xml-js), at most 150 parts, scrubbed like chat words; no markup, no paths, no formulas. The panel renders them read-only with **Download**, same member check.
...
`GET /api/phones` → `[{id, name, member, person, role, seen, online, reached, push}]` (`reached`: when the phone last reached this computer over each route, `{home?, tailscale?, relay?}`, worded by `adapter.reached()`; `push`: `on`, `off` (the person said no on the phone), `missing`, or absent); `POST /api/phones/pair {role: 'control'|'view'}` → `{qr, expires, urls}`;
`DELETE /api/phones/:id`; `GET /api/phones/link`, `PUT /api/phones/lan {on}` and `PUT /api/phones/relay {url}` → `{on, lan, pinned, tailscale, anywhere, hosts, relay, relayStatus, push, asking: [{id, name, words, role}]}`. `push` is `missing` when a phone's app build has no Android push credential or Expo refused it, else `ready`; `adapter.pushWords()` says it once.
...
Over the link only, for the phone itself: `GET /api/reach {via, ip}` → `{urls, anywhere, peer, reached}` (every address it can reach this computer at now: home network, Tailscale, the relay; the phone adds each to its grant, and gets the same list live as a `link.urls` event whenever it changes. `via` is the route it came by (`home`, `tailscale` or `relay`) and `ip` its own Tailscale address; `peer` says whether this computer's Tailscale has that address as a peer. The phone keeps `anywhere`, `peer` and `reached` for when it can't reach the computer, and `adapter.away()` turns them, with what it sees for itself and a bounded `adapter.knock()` on the computer's address, into what was observed and what to try) and `POST /api/push {expo} | {missing} | {off} | {web}` (its Expo push token, kept by crewd, which sends through Expo; `missing`: this app build has no push credential; `off`: the person said no; a browser's `web` address is kept on the relay). Every push says only "Crewhouse has news": a question for that person, their job finished, failed or not sure it worked, or Chief speaking to them. In their quiet hours the push is held (kept in the store) and one is sent when they end, however much came in.
