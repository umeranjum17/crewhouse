...
Over the link only, for the phone itself: `GET /api/reach` → `{urls}` (every address it can reach this computer at now: home network, Tailscale, the relay; the phone adds each to its grant, and gets the same list live as a `link.urls` event whenever it changes) and `POST /api/push {expo} | {web}` (its push address, kept on the relay). Every push says only "Crewhouse has news": a question for that person, their job finished or failed, or Chief speaking to them, never in their quiet hours.
