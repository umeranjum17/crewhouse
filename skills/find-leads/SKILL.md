...
## 0. Setup check (free)
Run `treg balance`. If `treg` is missing or not signed in, stop and tell the person exactly this:
...
Until then you can still price a job: `https://treg.to/catalog/search?q=<job>` is public (WebFetch).

## 1. Price it (free)
- `treg catalog search "<the job in plain words>"`, then `treg catalog get <id>` for inputs and price.
...
## 2. Ask, with the price on the command
Every call is an approval ask; the person sees the command, so the cap goes right after the id:

```
treg call treg.people.phone.find --header 'X-Treg-Route-Max-Cost: 0.05' --method POST --data '{"full_name":"Jane Doe","domain":"example.com"}'
```
...
- Finish with `treg balance` and report the real total spent.
