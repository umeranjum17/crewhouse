...
5. Title or end card: `ffmpeg -f lavfi -i color=c=0xF7F4EF:s=1920x1080:d=2.5 -vf "drawtext=text='Title':fontcolor=0x1E1A16:fontsize=96:x=(w-tw)/2:y=(h-th)/2" card.mp4`. If drawtext wants a font, `fc-match -f '%{file}' 'DejaVu Sans:bold'` prints one to pass as `fontfile=`.
