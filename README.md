...
./crewhouse setup     # installs dependencies, builds the web app, checks your machine
...
- Per-bot desktops: each bot's own display and browser, watch and take over through [desklink](https://www.npmjs.com/package/@desklink/host). The Screen tab is a placeholder.
- Codex turns. The runner starts `codex` with its `notify` hook wired in, but this path is untested.
