...
Local-first, open-source crew of persistent AI helpers run by Chief on your own AI account (your ChatGPT, or Grok, Meta Muse and others).
...
Open the address. Chief introduces himself and asks how to address you, then **Sign in with ChatGPT**: a code to type on ChatGPT's own page, and you're in. Then try:
...
- **Thinks with**: each bot has an order of AI accounts on its **Thinks with** tab, ChatGPT by default. When an account hits its limit ("resting until 3:40 pm"), the task carries on with the next one, in the same session file, conversation and all. Chief can also pick an account for a single task.
...
- **A household.** One person needs nothing new. Add someone under **Settings, People** and they get their own thread with Chief (who asks how to address them), their own bots, tasks and questions, and quiet hours during which a bot's question waits instead of holding the bot (standing answers still apply). The screen's owner is picked under **Who is using this screen?**; it is a view, not a login.
  - **Each person runs on their own AI accounts.** The companies' terms forbid making your account available to anyone else, so crewd never lends one. Each person, the owner too, has their own engine runtime and credential file (`~/.local/state/crewhouse/people/<id>/engine/auth.json`), and signs in from the app (`src/accounts.ts`): **Sign in with ChatGPT** shows a code and ChatGPT's own page, and finishes by itself. Grok (SuperGrok or X Premium), Meta Muse, GitHub Copilot, Kimi and OpenRouter sign in the same way; Gemini takes a free key pasted from Google's page. Claude is not offered: Anthropic allows its subscriptions only in its own apps.
  - A sign-in that fails, is declined, can't be reached, runs out of time or is cancelled ends signed out, with one plain sentence and one next step; a browser sign-in that can't come back falls back to a code by itself. Sign-ins are refreshed in the background, and one that lapses is signed out and Chief says so once.
...
- **Connections** (`src/connections.ts`): each person connects their own apps under **Settings, Connections**, on the app's own page, and every helper working for them can then use them, through the gate (reading runs silently; changing or sending asks). Notion and Canva register Crewhouse by themselves (OAuth dynamic client registration), so they need no setup. Gmail, Calendar and Drive are one Google connection through the household's own Google app: the owner creates a Google Cloud OAuth client ("Desktop app") once and puts it in `~/.local/state/crewhouse/apps.json` as `{"google": {"id": "…", "secret": "…"}}`. Tokens stay in each person's own folder (`people/<id>/connections.json`, mode 600), are refreshed in the background, and never reach a bot or the app screen. Outlook is coming.
...
- A card that asks you to connect an app in the middle of a task; today you connect under Settings and the next task can use it. Outlook, Spotify and phone photos. Setting up the household Google app from inside the app.
